...
from DMFNER.modeling_albert import AlbertModel, AlbertConfig
from DMFNER.modeling_bert import BertConfig, BertModel
from DMFNER.modeling_roberta import RobertaConfig, RobertaModel
from DMFNER.modeling_xlm_roberta import XLMRobertaConfig
from transformers.modeling_utils import PreTrainedModel
from DMFNER import util
import logging
from DMFNER import models_add
...
class SpanAttentionLayer(nn.Module):
    def __init__(self, d_model=768, d_ffn=1024, dropout=0.1, activation="relu", n_heads=8, self_attn=True,
                 cross_attn=True):
        super().__init__()
    ...
    def forward(self, tgt, pos, src, mask):
        ...
        if self.cross_attn_bool:
            ...
            tgt2 = self.cross_attn(q.transpose(0, 1), k.transpose(0, 1), v.transpose(0, 1),
                                   key_padding_mask=~mask if mask is not None else None)[0].transpose(0, 1)
...
class DiffusionNER(PreTrainedModel):
    ...
    def __init__(
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=1024, out_channels=64, kernel_size=3, padding=1),
            nn.GELU(),
            nn.Conv1d(in_channels=64, out_channels=32, kernel_size=3, padding=1),
            nn.GELU(),
            nn.Conv1d(in_channels=32, out_channels=1024, kernel_size=1)
        )
        self.DDZNet = models_add.DDZAttention(channel=1024, reduction=16)
        self.MSFblock = models_add.MSFblock(in_channels=1024)
        self.MSFblock3d = models_add.MSFblock3d(in_channels=1024)
        self.addAttention = models_add.EfficientAdditiveAttnetion(in_dims=1024, token_dim=1024)
        ...
        if self.span_attn_layers > 0:
            ...
            spanattentionlayer = SpanAttentionLayer(d_model=config.hidden_size, self_attn=not wo_self_attn,
    ...
    def model_predictions(self, span, h_token, h_token_lstm, timestep, token_masks, x_self_cond=None,
        ...
        outputs_logits, outputs_span, left_entity_token_p, right_entity_token_p = self.head(x_span, h_token,
                                                                                            h_token_lstm, timestep,
                                                                                            token_masks)
    ...
    def forward(self,
        ...
        h_token_add = self.addAttention(h_token_lstm)
        h_token_cnnp1 = h_token.permute(0, 2, 1)
        # h_token_cnnp2 = h_token_add.permute(0, 2, 1)
        h_token_cnn = self.cnn(h_token_cnnp1)
        h_token_cnn = h_token_cnn.permute(0, 2, 1)

        # h_token_fuse = self.MSFblock(h_token_lstm, h_token_lstm2)
        # h_token_fuse = self.MSFblock3d(h_token_lstm, h_token_gru, h_token_add)
        h_token_fuse = self.MSFblock3d(h_token_lstm, h_token_add, h_token_cnn)
        # h_token_fuse = h_token_lstm

        # Prepare Proposals.
        if not self.training:
            results = self.ddim_sample(h_token, h_token_fuse, token_masks)
            return results
        ...
        if self.training:
            ...
            d_spans, noises, t = self.prepare_targets(entity_spans, entity_types, entity_masks, token_masks,
                                                      meta_doc=meta_doc)
            t = t.squeeze(-1)
            outputs_class, outputs_span, left_entity_token_p, right_entity_token_p = self.head(d_spans, h_token,
                                                                                               h_token_fuse, t,
                                                                                               token_masks)
    ...
    def backbone(self,
        ...
        if self.lstm_layers > 0:
            ...
            # """加的lstm"""
            # # token_count = token_masks.long().sum(-1,keepdim=True)
            # h_token_lstm2 = nn.utils.rnn.pack_padded_sequence(input=h_token,
            #                                                   lengths=token_count.squeeze(-1).cpu().tolist(),
            #                                                   enforce_sorted=False, batch_first=True)
            # h_token_lstm2, (_, _) = self.lstm2(h_token_lstm2)
            # h_token_lstm2, _ = nn.utils.rnn.pad_packed_sequence(h_token_lstm2, batch_first=True)
    ...
    def left_right_type(self, h_token, h_token_lstm, span_mask, timestep_embeddings, span, token_count, token_masks):
        ...
        if self.span_attn_layers > 0:
            ...
            entity_spans_pool = self.DDZNet(entity_spans_pool)
